...
async function checkEstoniaSettlementNationalityEducation() {
  ...
  const unknown = nationalityValue("6");
  assertPositive(nationalityTotal, "Estonia RV022U nationality total");
  assertEqual(estonian + russian + unknown <= nationalityTotal, true, "Estonia nationality named groups plus unknown do not exceed total");
  assertPositive(unknown, "Estonia RV022U unknown nationality total");
  assertQuotaSum([estonian, russian, nationalityTotal - estonian - russian - unknown], 1000, "Estonia nationality quotas sum to sample size after unknown exclusion");
