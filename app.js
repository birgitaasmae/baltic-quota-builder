...
function ethnicityRows(nativeLabel, russianLabel, otherLabel, nativePopulation, russianPopulation, totalPopulation, unknownPopulation = 0) {
  ...
  const unknown = Number(unknownPopulation) || 0;
  return [
...
function nationalitySourceNoteWithExclusion(sourceNote, extra = "") {
  return `${sourceNote} Unknown or not indicated nationality is excluded from calculation.${extra ? ` ${extra}` : ""}`;
}
...
function describeAgeGroupCoverage(groups) {
  ...
  if (last.from === 85 && last.to === 99) {
    return from <= 84 ? `${from}-84 and 85+ (up to 99 years)` : "85+ (up to 99 years)";
  }
  return `${from}-${last.to}`;
}
...
function formatPercentUnits(units, decimals = PERCENT_DECIMALS) {
  return `${(units / (10 ** decimals)).toFixed(decimals)}%`;
}

function percentageStrings(populations, decimals = PERCENT_DECIMALS) {
...
function totalPercentString() {
  return formatPercentUnits(100 * (10 ** PERCENT_DECIMALS));
}
...
async function fetchLithuaniaNationality(year) {
  ...
  return {
    ...
    sourceNote: data.nationalitySourceNote || `State Data Agency of Lithuania / Official Statistics Portal: ${LITHUANIA_NATIONALITY_SOURCE_TITLE}, SDMX flow S3R167_M3010215_1. Whole-country ethnicity distribution. Unknown or not indicated nationality is excluded from calculation.`
  };
}
...
function addExportRows(section, headers, rows, footer = null, note = null) {
  ...
  if (note) state.rowsForExport.push([note]);
  ...
  try {
    ...
    if (regionLevel > 0 && population.regional.size) {
      ...
      if (nonZero.length) {
        ...
        renderTable(els.regionTable, ["Region", "Population", "%", "Quota"], regionRows, ["Total", fmt(regionalTotal), totalPercentString(), sampleSize]);
        setMeta(els.regionMeta, populationNote, minAge, maxAge);
        addExportRows("Regional Distribution", ["Region", "Population", "%", "Quota"], regionRows, ["Total", fmt(regionalTotal), totalPercentString(), sampleSize], populationNote);
    ...
    if (sexes.length === 2) {
      ...
      const crossNote = `${populationNote}; shown as a sex by age cross table.`;
      setMeta(els.crossMeta, crossNote, minAge, maxAge);
      addExportRows("Sex x Age Cross Table", crossHeaders, crossRows, crossFooter, crossNote);
